...
export default function ViewTemplatePage() {
  ...
  const roleSlug = user?.role?.replace(/_/g, '-') ?? 'branch-admin'; // ← ADDED
    ...
    <div style={{ padding: '32px 24px', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '20px 24px', maxWidth: 680 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          ...
          <Link href={`/${roleSlug}/template-management`} style={{ padding: '8px 18px', background: '#F1F5F9', color: '#1E293B', border: '1px solid #E2E8F0', borderRadius: 6, fontSize: 13, fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>← All Templates</Link>
        </div>
      </div>
    </div>
  );
