...
export default function ViewTemplatePage() {
  ...
  const handleDeleteObjective = async (ci: number, oi: number) => {
    ...
    if (!obj.isNew) {
      try {
        await fetch(
          `${API}/api/templates/${templateId}/objectives/${obj.id}`,
          { method: 'DELETE' }
        );
      } catch {}
    }
  ...
  if (error) return (
    <div style={{ padding: '32px 24px', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '20px 24px', maxWidth: 680 }}>
        ...
        <div style={{ fontFamily: 'monospace', fontSize: 12.5, color: '#7F1D1D', background: '#FFF1F2', padding: '10px 14px', borderRadius: 6, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
          {error}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          <button onClick={fetchTemplate} style={{ padding: '8px 18px', background: '#2563EB', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
            Retry
          </button>
          <Link href={`/${roleSlug}/template-management`} style={{ padding: '8px 18px', background: '#F1F5F9', color: '#1E293B', border: '1px solid #E2E8F0', borderRadius: 6, fontSize: 13, fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
            ← All Templates
          </Link>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            <Link href={`/${roleSlug}/template-management`} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', border: '1px solid #E2E8F0', borderRadius: 6, background: '#F8F9FC', textDecoration: 'none', fontSize: 13, color: '#1E293B' }}>
              <ArrowLeft size={14} />Back
            </Link>

            {!editMode ? (
              <div title={!canEdit ? editBlockedReason : undefined} style={{ display: 'inline-block' }}>
                <button
              ...
              <button onClick={handleCancel} style={{ padding: '8px 16px', borderRadius: 6, background: '#F1F5F9', border: '1px solid #E2E8F0', cursor: 'pointer', fontSize: 13, color: '#1E293B', fontWeight: 600 }}>
                Cancel
              </button>
            )}
          </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '16px 20px', marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 16 }}>
            {[
              ...
              <div key={i}>
                <div style={{ fontSize: 11, color: '#4A5565', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {s.label}
                </div>
                {s.badge
                  ? <span style={{
                      background: s.badge === 'active' ? '#2563EB' : '#FEF3C7',
                      color: template.status === 'active' ? '#fff' : '#92400E',
        ...
        {editMode && (
          <div style={{ background: '#FEF9C3', border: '1px solid #FDE047', borderRadius: 8, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: '#854D0E' }}>
            <strong>Edit Mode</strong> — Locked objectives cannot be deleted or have their weights changed.
            New objectives require name, weight, control type, and KPI scale.
            Total weight must equal <strong>100%</strong>.
          </div>
        )}
        ...
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden', marginBottom: 20 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 14, fontSize: 12, color: '#4A5565' }}>
              {[
                { label: 'Locked',   bg: '#DBEAFE', border: '#BFDBFE' },
                { label: 'Editable', bg: '#fff',    border: '#D1D5DC' },
              ].map(l => (
          ...
          <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: editMode ? 680 : 560 }}>
              ...
              <tbody>
                        ...
                        return (
                          <tr
                            ...
                            style={{
                              ...
                              outline:      missing && objectiveError ? '2px solid #FCA5A5' : 'none',
                            }}
                          >
                            ...
                            <td style={tdStyle('center')}>
                              {editMode && isNew
                                ? <select
                                    value={obj.control_type}
                                    onChange={e => updateObj(ci, oi, { control_type: e.target.value })}
                            ...
                            <td style={tdStyle('left')}>
                              {editMode && isNew
                                ? <ScalePicker
                                    value={obj.kpi_scale ?? undefined}
                                    onChange={v => updateObj(ci, oi, { kpi_scale: v })}
                                    hasError={!!objectiveError}
                                  />
                            ...
                            {editMode && (
                              <td style={tdStyle('center')}>
                                {!isLocked && (
                                  <button
                                    onClick={() => handleDeleteObjective(ci, oi)}
                                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#DC2626', padding: 4, borderRadius: 4, display: 'inline-flex', alignItems: 'center' }}
                                  >
                                    <Trash2 size={15} />
                                  </button>
                                )}
                ...
                {/* Total weight footer (view mode only) */}
                {!editMode && (
        ...
        {/* ── Employee Assignment (view mode only) ──────────────── */}
        {!editMode && (
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '20px', marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
              <div>
                ...
                <p style={{ margin: '2px 0 0', fontSize: 12.5, color: '#64748B' }}>
                  Search and assign your direct reports to this evaluation template
                </p>
              </div>
            ...
            <div ref={searchRef} style={{ position: 'relative', marginBottom: 12 }}>
              ...
              {showDropdown && !empSearching && empSearch.trim().length > 0 && empResults.length === 0 && !selectedEmployee && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 90, background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.10)', zIndex: 100, padding: 14, fontSize: 13, color: '#94A3B8', marginTop: 4, textAlign: 'center' }}>
                  No team members found matching &quot;{empSearch}&quot;
                </div>
              )}
            </div>
            ...
            {showConflictBox && conflictEmployee && (
              <div style={{ border: '1px solid #FDE68A', background: '#FFFBEB', borderRadius: 10, padding: '13px 15px', marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    onClick={() => commitAssign(conflictEmployee)}
                    disabled={assigning}
                    style={{ padding: '6px 14px', borderRadius: 7, border: 'none', cursor: 'pointer', background: '#D97706', color: '#fff', fontSize: 12, fontWeight: 700 }}
                  >
                    {assigning ? 'Assigning…' : 'Yes, reassign'}
                  </button>
                  <button
                    onClick={() => { setShowConflictBox(false); setConflictEmployee(null); setSelectedEmployee(null); setEmpSearch(''); }}
              ...
              <div style={{ border: '1px solid #E2E8F0', borderRadius: 8, overflow: 'hidden' }}>
                ...
                >
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    Assigned Employees
                    <span style={{ background: '#2563EB', color: '#fff', borderRadius: 20, padding: '1px 8px', fontSize: 11, fontWeight: 600 }}>
                ...
                {showAllocated && assignedEmployees.map((emp, i) => (
                  <div
                    key={emp.id}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: i % 2 === 0 ? '#fff' : '#F9FAFB', borderTop: '1px solid #F1F5F9', flexWrap: 'wrap' }}
                  >
        ...
        {editMode && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 13, flex: 1, color: currentTotal === 100 ? '#16A34A' : '#DC2626' }}>
              {currentTotal === 100
                ? '✅ Total weight is 100% — ready to save.'
                : `⚠️ Total weight: ${currentTotal}% (must be 100%)`
              }
            </span>
            <button
              onClick={handleCancel}
              style={{ padding: '10px 20px', borderRadius: 6, background: '#F1F5F9', border: '1px solid #E2E8F0', cursor: 'pointer', fontSize: 13, color: '#1E293B', fontWeight: 600 }}
            >
              Cancel
            </button>
            <button
              ...
              disabled={saving}
              style={{ padding: '10px 24px', borderRadius: 6, background: saving ? '#93C5FD' : '#16A34A', border: 'none', cursor: saving ? 'not-allowed' : 'pointer', fontSize: 13, color: '#fff', fontWeight: 600 }}
            >
