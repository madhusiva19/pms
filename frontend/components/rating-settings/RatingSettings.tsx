...
// Backend resolves the most specific period row for the requesting user
...
interface OrgHierarchy {
  countries:       OrgItem[];      // empty for Country Admin — CA cannot change their own period
  branches:        BranchItem[];
...
// We compare by rating_end date rather than pms_year/period label because the
...
// KEY DESIGN: one checkbox = one admin's period row only (no cascading writes).
// Selecting a country filters the branches list but does NOT auto-select branches.
// Deselecting a parent prunes its children from selection to prevent stale
// invisible selections from being submitted accidentally.
...
function CascadeScopePicker({
  ...
  return (
    <div>
      ...
      {/* Include myself — HQ only. Updates the global (all-NULL) period row
          which changes the HQ Admin's own rating window without affecting
          any other user (they resolve to a more specific scoped row). */}
      {isHQ && (
        <label style={{
...
// ── EDIT PERIOD MODAL ─────────────────────────────────────────────
// HQ Admin can update any admin's period; Country Admin can only update
// admins under their own country. CA's own period is set exclusively by HQ.
...
function EditPeriodModal({ period, pmsYear, currentStart, currentEnd, evaluatorId, onClose, onSaved }: {
  ...
  // When true, the global (all-NULL) row is also updated — changes HQ Admin's
  // own window without affecting any other user (they resolve to a scoped row).
...
export default function RatingSettings() {
  ...
  const fetchRatingStatuses = useCallback(async (
    ...
    try {
      ...
      // On failure mark all as pending so we don't silently show incorrect "Submitted" statuses
  ...
  //   1. URL param override (?year=YYYY&period=H1) — must match BOTH fields to
  //      avoid confusing H1 2025 with H1 2026.
  //   2. Currently open window from the API.
  //   3. getMostRecentPastPeriod fallback.
