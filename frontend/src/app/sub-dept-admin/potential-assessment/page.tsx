...
/**
 * Sub Dept Admin — Potential Assessment page
 * Section A: My Self-Assessment | Section B: Employee team list
 */
...
export default function SubDeptAdminPotentialAssessmentPage() {
  ...
  return (
    <div className="flex flex-col gap-10 max-w-[1225px] mx-auto w-full">
      <Breadcrumb items={[{ label: 'Home', href: '/sub-dept-admin/dashboard' }, { label: 'Potential Assessment' }]} />
      ...
      <section className="flex flex-col gap-4">
        ...
        {status === 'completed' && assessment ? <CompletedSummary assessmentData={assessment} viewerRole="appraisee" />
        : status === 'pending_supervisor' ? <div className="bg-[#FEF9C3] border border-[#FDE68A] rounded-xl px-5 py-4 text-[13.5px] text-[#92400E] font-medium">Your self-assessment has been submitted. Awaiting Dept Admin Review.</div>
        : <SelfAssessmentForm employeeId={user.id} supervisorId={assessment?.supervisor_id ?? (user as any).manager_id ?? ''} role="sub_dept_admin" cycle={cycle?.name ?? ''} currentStatus={status} assessmentData={assessment} onSubmitSuccess={() => reload()} />}
      </section>
                ...
                {subordinates.map((sub) => {
                  ...
                  return (
                    <tr key={sub.id} className="border-b border-[#F1F5F9] hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className="px-5 py-4"><button disabled={!canReview} onClick={() => router.push(`/sub-dept-admin/potential-assessment/${sub.id}`)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-medium transition-colors ${canReview ? 'bg-[#1E3A8A] text-white hover:bg-[#1E40AF]' : 'bg-[#F1F5F9] text-[#94A3B8] cursor-not-allowed'}`}>Review <ChevronRight className="w-3.5 h-3.5" /></button></td>
                    </tr>
                  );
                })}
              </tbody>
