...
export default function HQAdminReconsiderationReviewPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const msg = showOverride && (overrideAbility || overrideAspiration || overrideLeadership || overrideTalentBlock)
        ? 'Scores updated and reconsideration approved.'
        : 'Reconsideration approved. Assessment marked as completed.';
      alert(msg);
      router.push('/hq-admin/potential-assessment');
    } catch (err: any) {
  ...
  const handleReject = async () => {
    ...
    setSubmitting(true);
    try {
      await reconsiderationApi.review(assessmentId, 'reject', {
        reviewerId: user.id,
        rejectionNote,
      });
    ...
    <div className="flex flex-col gap-8 max-w-[1225px] mx-auto w-full">
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] p-6 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-end gap-3 pt-1">
          <button type="button"
            onClick={() => router.push('/hq-admin/potential-assessment')}
