...
export default function EmployeePotentialAssessmentPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'reconsideration_requested' || status === 'reconsideration_rejected' || status === 'completed') {
      reconsiderationApi.getStatus(assessment.id)
        .then(setReconsideration)
        .catch(() => setReconsideration(null));
    }
  ...
  const isCompletedOrReviewed =
    status === 'completed' || status === 'supervisor_reviewed' as string;
        ...
        <div className="flex flex-col gap-4 bg-white rounded-xl border border-[#E5E7EB] p-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              {reconsideration?.reviewed_at && (
                <p className="text-[12.5px] text-[#94A3B8]">
                  Reviewed on: <span className="text-[#64748B] font-medium">{formatDate(reconsideration.reviewed_at)}</span>
                </p>
              )}
            </div>
          )}
          ...
          {status === 'completed' && hasBeenReconsidered && (
            <div className="flex flex-col gap-2">
              ...
              {reconsideration?.reviewed_at && (
                <p className="text-[12.5px] text-[#94A3B8]">
                  Approved on: <span className="text-[#64748B] font-medium">{formatDate(reconsideration.reviewed_at)}</span>
                </p>
              )}
            </div>
          )}
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-[13.5px] text-[#64748B] mt-1">
                  You may request a reconsideration by a senior supervisor. This can only be done once.
                </p>
              </div>
