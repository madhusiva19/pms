...
export default function DeptAdminReportDetailPage() {
  ...
  const fetchPeriodData = async (tab: ReportType, summaryData: BranchDashboardSummary) => {
    ...
    const [metricsResult, bellCurveResult, insightsResult] = await Promise.allSettled([
      metricsApi.get({ period_type: tab, year: reportYear!, scope: 'sub_department', scope_id: subDeptId }),
      bellCurveApi.getLive({ period_type: tab, year: reportYear!, scope: 'sub_department', scope_id: subDeptId }),
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const fileName = `${teamName}-${activeTab === 'mid_year' ? 'Mid-Year' : 'Year-End'}-${reportYear!}.pdf`;
