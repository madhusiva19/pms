...
/**
 * Dept Admin — Potential Assessment page
 * Section A: My Self-Assessment | Section B: Sub Dept Admin team list
 */
...
export default function DeptAdminPotentialAssessmentPage() {
  ...
  return (
    <div className="flex flex-col gap-10 max-w-[1225px] mx-auto w-full">
      <Breadcrumb items={[{ label: 'Home', href: '/dept-admin/dashboard' }, { label: 'Potential Assessment' }]} />
      ...
      <section className="flex flex-col gap-4">
        ...
        : status === 'pending_supervisor' ? <div className="bg-[#FEF9C3] border border-[#FDE68A] rounded-xl px-5 py-4 text-[13.5px] text-[#92400E] font-medium">Your self-assessment has been submitted. Awaiting Branch Admin Review.</div>
        : <SelfAssessmentForm employeeId={user.id} supervisorId={assessment?.supervisor_id ?? (user as any).manager_id ?? ''} role="dept_admin" cycle={cycle?.name ?? ''} currentStatus={status} assessmentData={assessment} onSubmitSuccess={() => reload()} />}
      </section>
