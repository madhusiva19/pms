...
export default function BranchAdminReviewPage() {
  const { user, loading: authLoading } = useAuth();
  const router = useRouter();
  const params = useParams();
  const deptAdminId = params?.deptAdminId as string;
    ...
    <div className="flex flex-col gap-8 max-w-[1225px] mx-auto w-full px-8 py-6 pb-10">
      ...
      <div className="w-full rounded-xl border border-[#BEDBFF] px-4 py-3 flex items-center justify-between" style={{ background: 'linear-gradient(90deg, #EFF6FF 0%, #F3F4F6 100%)' }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col"><span className="text-[12.7px] text-[#4A5565]">Selected Dept Admin</span><span className="text-[18px] font-semibold text-[#101828]">{appraisee?.full_name ?? '—'}</span></div>
        ...
        <button onClick={() => router.push('/branch-admin/potential-assessment')} className="flex items-center gap-2 px-3 py-[7px] bg-[#F9FAFB] border border-[#E5E7EB] rounded-md text-[13px] font-medium text-[#1E293B] hover:bg-gray-100"><ChevronLeft className="w-4 h-4" /> Back</button>
      </div>
