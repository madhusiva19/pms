...
/**
 * Country Admin — Review detail page for a Branch Admin
 */
...
export default function CountryAdminReviewPage() {
  const { user, loading: authLoading } = useAuth();
  const router = useRouter();
  const params = useParams();
  const branchAdminId = params?.branchAdminId as string;

  const [cycle, setCycle] = useState<AppraisalCycle | null>(null);
  const [appraisee, setAppraisee] = useState<{ full_name: string; emp_id?: string; iata_branch_code?: string } | null>(null);
  ...
  const loadData = async (activeCycle: AppraisalCycle) => {
    const [data, subs] = await Promise.all([
      potentialAssessmentApi.getForEmployee(branchAdminId, String(activeCycle.pms_year), user!.id),
    ...
    <div className="flex flex-col gap-8 max-w-[1225px] mx-auto w-full px-8 py-6 pb-10">
      ...
      <div className="w-full rounded-xl border border-[#BEDBFF] px-4 py-3 flex items-center justify-between"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-[12.7px] text-[#4A5565]">Selected Branch Admin</span>
            <span className="text-[18px] font-semibold text-[#101828]">{appraisee?.full_name ?? '—'}</span>
            {appraisee?.iata_branch_code && <span className="text-[12px] text-[#64748B]">IATA: {appraisee.iata_branch_code}</span>}
          </div>
        ...
        <button onClick={() => router.push('/country-admin/potential-assessment')}
          className="flex items-center gap-2 px-3 py-[7px] bg-[#F9FAFB] border border-[#E5E7EB] rounded-md text-[13px] font-medium text-[#1E293B] hover:bg-gray-100">
      ...
      {(status === 'not_started' || status === 'pending_self') ? (
        <div className="bg-[#FEF9C3] border border-[#FDE68A] rounded-xl px-5 py-4 text-[13.5px] text-[#92400E]">
          This Branch Admin has not yet submitted their self-assessment. Review will be available once they submit.
        </div>
