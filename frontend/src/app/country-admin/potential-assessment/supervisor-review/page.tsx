...
export default function CountryAdminTeamReviewPage() {
    ...
    <div className="flex flex-col gap-10 max-w-[1225px] mx-auto w-full">
      ...
      <section className="flex flex-col gap-4">
        <div className="flex flex-col gap-1 pb-3 border-b border-[#E5E7EB]">
          ...
          <p className="text-[13.5px] text-[#64748B]">Pending reconsideration requests from Dept Admins awaiting your review as senior supervisor.</p>
        </div>
        <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
          <table className="w-full border-collapse">
            <thead>
              ...
              {reconsiderations.length === 0 ? (
                ...
                reconsiderations.map((recon) => (
                  <tr key={recon.assessment_id} className="border-b border-[#F1F5F9] hover:bg-[#FAFAFA] transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <button type="button"
                        onClick={() => router.push(`/country-admin/potential-assessment/reconsideration/${recon.assessment_id}`)}
