...
/**
 * Country Admin — Potential Assessment page
 * Section A: My Self-Assessment
 * Section B: Branch Admin team list + review buttons
 */
...
import type { AppraisalCycle, SubordinateAssessmentSummary, AssessmentStatus, PotentialAssessment } from '@/types';
...
export default function CountryAdminPotentialAssessmentPage() {
  const { user, loading: authLoading } = useAuth();
  const router = useRouter();
  const { cycle, assessment, status, loading: selfLoading, error: selfError, reload } = useAssessmentData(user?.id, user?.id);

  const [teamCycle, setTeamCycle] = useState<AppraisalCycle | null>(null);
  ...
  return (
    <div className="flex flex-col gap-10 max-w-[1225px] mx-auto w-full">
      <Breadcrumb items={[{ label: 'Home', href: '/country-admin/dashboard' }, { label: 'Potential Assessment' }]} />
      ...
      <section className="flex flex-col gap-4">
        ...
        {status === 'completed' && assessment ? (
          ...
          <div className="bg-[#FEF9C3] border border-[#FDE68A] rounded-xl px-5 py-4 text-[13.5px] text-[#92400E] font-medium">
            Your self-assessment has been submitted. Awaiting HQ Admin Review.
          </div>
        ) : (
          <SelfAssessmentForm
            employeeId={user.id}
      ...
      {/* Section B — Branch Admin Team */}
      <section className="flex flex-col gap-4">
        <div className="flex flex-col gap-1 pb-3 border-b border-[#E5E7EB]">
          <h2 className="text-[18px] font-semibold text-[#101828]">Branch Admin Assessments</h2>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {subordinates.length === 0 && (
                  <tr><td colSpan={5} className="px-5 py-12 text-center text-[#94A3B8] text-[14px]">No Branch Admins found under your country.</td></tr>
                )}
                {subordinates.map((sub) => {
                  ...
                  return (
                    <tr key={sub.id} className="border-b border-[#F1F5F9] hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[12px] font-medium border ${badge.cls}`}>{badge.label}</span>
                      </td>
                      <td className="px-5 py-4">
                        <button disabled={!canReview}
                          onClick={() => router.push(`/country-admin/potential-assessment/${sub.id}`)}
