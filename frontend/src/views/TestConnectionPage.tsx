...
export default function TestConnection() {
  ...
  const [teamMembers, setTeamMembers] = useState<TestTeamMember[]>([]);
    ...
    const testConnection = async () => {
      try {
        ...
        if (membersError) {
          ...
          setTeamMembers((membersData || []) as TestTeamMember[]);
        ...
        if (approvalsError) {
          ...
          setApprovals((approvalsData || []) as TestApproval[]);
        ...
        setSupabaseStatus(`❌ Connection Error: ${message}`);
        setError(message);
        setLoading(false);
      }
    };
