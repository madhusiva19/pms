...
from utils.helpers import resolve_emp_ids_by_scope, fetch_summaries_for_ids, calculate_bell_curve_from_scores
...
def get_comparison_live(year: int, scope: str, scope_id: str) -> list:
    ...
    h1_rows = fetch_summaries_for_ids(emp_ids, year, 'H1', columns='total_score')
    h2_rows = fetch_summaries_for_ids(emp_ids, year, 'H2', columns='total_score')

    h1_by_range = {d['rating_range']: d['employee_count'] for d in calculate_bell_curve_from_scores(h1_rows)}
    h2_by_range = {d['rating_range']: d['employee_count'] for d in calculate_bell_curve_from_scores(h2_rows)}
